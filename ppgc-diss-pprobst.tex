...
If GBFS is guided by a blind heuristic function (equal value for all the expanded states but goal-aware and safe), tie-breaks by larger $g$-value, uses ideal preferred operators, exclusively expands states from the preferred queue since the beginning of the search, includes the initial state $s_{0}$ in the preferred queue, and assumes all operators of the task have a constant cost $c \in \mathbb{R}^{>0}$, then GBFS only expands states part of a unique optimal path.
\end{property}

\begin{proof}
We will prove the property using induction on the number of expansions performed by GBFS.

\textit{Base case}: Let $k = 0$. Initially, the only state in the preferred queue $\mathcal{Q}$ is the initial state $s_{0}$. GBFS expands states exclusively from the preferred queue. Since $s_{0}$ is in $\mathcal{Q}$, GBFS will start by expanding $s_{0}$. This expansion satisfies the property since $s_{0}$ is the only state expanded, and $s_{0}$ is part of any optimal path.

\textit{Inductive hypothesis}: Assume that after $k$ expansions, GBFS has only expanded states that are part of a unique optimal path.

\textit{Inductive step:} Consider the $(k+1)$-th expansion step. GBFS selects a state $s \in \mathcal{Q}$ to expand, with $g_{s} = (k+1) \cdot c$ because all operators have a constant cost $c$. By the induction hypothesis, all states expanded in the first $k$ steps are part of a unique optimal path.

By \cref{def:ideal_preferred_operator}, every ideal preferred operator generates a state with the shortest distance to the goal among all successors. Therefore, when expanding state $s$, GBFS will select an ideal preferred operator that leads to a successor state $t$ such that $\hstarp{t} = \min_{s' \in \sucs(s)} \hstarp{s'}$, which is then inserted into $Q$. Also, the blind heuristic is safe; thus, $t$ is not discarded by GBFS since $h(t) \neq \infty$ (i.e., $t$ is not a dead-end). The $g$-value of $t$ is $g_{t} = g_{s} + c$. Since all the states in $\mathcal{Q}$ have equal and finite $h$-values, and GBFS tie-breaks by larger $g$-value, GBFS will expand $t$ over any other state in the $\mathcal{Q}$, as they all have smaller or equal $g$-values than $g_{s}$.

By expanding $t$ from $\mathcal{Q}$, GBFS continues the search along a unique optimal path.
Since the $(k+1)$-th expansion step satisfies the property, and we have shown the base case, we can conclude that GBFS only expands states that are part of a unique optimal path.
\end{proof}
