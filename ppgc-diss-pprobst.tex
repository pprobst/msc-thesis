...
A planning task in STRIPS representation is defined by a tuple~$\Pi=\langle\mathcal{F},\mathcal{O},\mathcal{I},\mathcal{G}\rangle$. Here, $\mathcal{F}$~is a set of propositions, $\mathcal{O}$~is a set of operators, $\mathcal{I} \subseteq \mathcal{F}$~is an initial state, and $\mathcal{G}$~is a set of goal states. Each assignment $s \subseteq \mathcal{F}$ represents a state, and the set~$\mathcal{S}$ of all states over $\mathcal{F}$ is the state space.
