...
In this section we present two ways to represent a planning task. The first one, STRIPS~\cite{Fikes.Nilsson/1971}, represents a planning problem using propositional facts (Boolean variables), while the second one, \sas~\cite{Backstrom.Nebel/1995}, represents them using multi-valued state variables. The \sas representation is more commonly used among current planners.
...
A planning task in STRIPS representation~\cite{Fikes.Nilsson/1971} is defined by a tuple~$\Pi=\langle\mathcal{F},\mathcal{O},\mathcal{I},\mathcal{G}\rangle$. Here, $\mathcal{F}$~is a set of propositions, $\mathcal{O}$~is a set of operators, $\mathcal{I} \subseteq \mathcal{F}$~is an initial state, and $\mathcal{G}$~is a set of goal states. Each assignment $s \subseteq \mathcal{F}$ represents a state, and the set~$\mathcal{S}$ of all states over $\mathcal{F}$ is the state space.
...
Heuristics are typically derived from a model of the task, such as the \sas model introduced earlier, but can also be obtained by learning the map a state $s$ to its heuristic value $h(s)$, where the desired output of the NN can be either the direct cost-to-goal estimates or some form of encoding representing these estimates.
...
The current approach to extract preferred operators is the one implemented in the Fast Downward planning system~\cite{Helmert/2009}, where the preferred operators from FF are the current best.
