...
\begin{proof}
%Assume the task is solvable and that all the operators are unitary.
...
% If there are multiple shortest paths, then the state $s$ may produce $n > 1$ successors generated by ideal preferred operators, which are all inserted into $\mathcal{Q}$. The successors not generated by preferred operators are ignored.

%Now, consider the $(k+1)$-th expansion step. Since all the states in $\mathcal{Q}$ have equal and finite $h$-values, and GBFS tie-breaks by larger $g$-value, GBFS will prioritize expanding one of the $n$ successors of $s$, as they all have greater $g$-values than $g_{s}$. GBFS selects to expand $t \in \mathcal{Q}$ (one of the possible $n$ successors of $s$), with $g_{t} = g_{s} + 1$. Thus, the perfect cost-to-goal estimate of state $t$ is $\hstarp{t} = \hstarp{s} - 1$, i.e., an expansion of a state generated by an ideal preferred operator makes progress on an optimal path. %By the induction hypothesis, all states expanded in the first $k$ steps are part of an optimal path.
