...
GBFS in dead-end-free tasks only expands states on an optimal path by using a safe heuristic function, ideal preferred operators, tie-breaking by larger $g$-value, and exclusively expanding states from the preferred queue.
\end{property}

\ppi{Sound? Also, I don't think we need ``without dead-ends'' mentioned in the property. Isn't ``safe heuristic'' enough? Since GBFS does not expand states with infinite $h$-values.}
\begin{proof}
We will prove this property by contradiction.

Let $\mathcal{P}$ denote the set of all optimal paths from $s_{0}$ to $s^{*}$. Assume that when GBFS is run with the specified conditions, there exists a state $t$ that is expanded from the preferred queue but does not belong to any path $p \in \mathcal{P}$.
