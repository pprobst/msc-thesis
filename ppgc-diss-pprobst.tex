...
\Procedure{RelaxedPlanningGraph}{relaxed planning task $\Pi^{+}$}
  ...
  \While{$s^{*} \nsubseteq P_{k}$}
    \State $k \gets k + 1$
    \State \# \emph{Computes the action layer $O_{k}$ at layer $k$, i.e., all the actions}
    \State \# \emph{with the preconditions satisfied in the layer $P_{k}$}
    \State $O_{k} \gets \{o \in O\ \mid pre(o) \subseteq P_{k}\}$
    \State $P_{k} \gets P_{k-1}$
    \ForAll{$o \in O_{k}$}
      \State \# \emph{Computes the proposition layer $P_{k}$ at layer $k$}
...
\Procedure{RelaxedPlan}{relaxed planning graph $G^{+}$, goal facts $s^{*}$}
  ...
  \For{$k \gets M$ \textbf{to} $1$}
    \ForAll{$s_{k}^{*} \in S_{k}^{*}$}
      \State \# \emph{Selects the action $o$ that achieves the goal fact $s_{k}^{*}$}
      \State \# \emph{and appears for the first time in layer $k$}
      \State $o \gets$ $\text{firstlayer}(o, O_{k}) = k \mid s_{k}^{*} \in add(o)$
      \State $plan \cup \{o\}$
      \State \# \emph{Now add the preconditions of $o$ as sub-goals to $S^{*}$}
      \State \# \emph{in the layer where $p$ first appears}
      \ForAll{$p \in pre(o)$}
