...
In this study, we use a propositional representation of a state to learn heuristic functions and preferred operators~\cite{Ferber.etal/2020a,Yu.etal/2020,Ferber.etal/2022,OToole/2022,Bettker.etal/2022}. %We use the following notation from~\citet{Bettker.etal/2022}.
...
For example, let $\mathcal{F}$ be a set of facts, and let $f_i, f_j \in \mathcal{F}$ be two facts. We say that $f_i$ and $f_j$ are mutex if $\neg(f_i \land f_j)$ holds, i.e., $f_i$ and $f_j$ cannot both be true at the same time in any valid state of the planning problem. More generally, we express $\mutex(\mathcal{F})$ when the constraint $\sum_{f\in \mathcal{F}} [f]\leq 1$ must be satisfied within the states of the planning task.
...
With a boost value $n$, when a state with a lower $h$-value than any previously expanded state is encountered during the search, the preferred queue is used for the next $n$ expansions, as long as it has elements. The boost value is cumulative, so when the search progresses, $n$ expansions are added to the remaining number of subsequent expansions from the preferred queue.
...
An example of a simple fully-connected NN is shown in~\cref{fig:neural-network}. Some common activation functions include sigmoid, rectified linear units (ReLU), and tanh~(\cref{fig:activation-functions}).
...
If GBFS is guided by a blind heuristic function (equal value for all the expanded states but goal-aware and safe), tie-breaks by larger $g$-value, uses ideal preferred operators, exclusively expands states from the preferred queue since the beginning of the search, includes the initial state $s_{0}$ in the preferred queue, then GBFS expands at most the number of states part of an optimal path.
\end{property}

\begin{proof}
Assume the task is solvable and that all the operators are unitary. Hence, the length of an optimal path equals the number of expansions, and at each expansion, the $g$-value increases by one.
We will prove the property using induction on the number of expansions performed by GBFS.

\textit{Base case}: Initially, the only state in the preferred queue $\mathcal{Q}$ is the initial state $s_{0}$. GBFS expands states exclusively from the preferred queue. GBFS will start by expanding $s_{0}$, as $s_{0}$ is in $\mathcal{Q}$. Since the task is solvable and GBFS does not ignore $s_{0}$ because the blind heuristic is safe, then $\hstarp{s_{0}} \neq \infty$. This expansion satisfies the property since $s_{0}$ is the only state expanded, and $s_{0}$ is part of any optimal path.

\textit{Inductive hypothesis}: Assume that after $k$ expansions, GBFS has only expanded states part of an optimal path.

\textit{Inductive step:}
By \cref{def:ideal_preferred_operator}, every ideal preferred operator generates a state with the shortest distance to the goal among all successors. Therefore, when expanding state $s$ at the $k$-th expansion step, GBFS will select an ideal preferred operator that leads to a successor state $t$ such that $\hstarp{t} = \min_{s' \in \sucs(s)} \hstarp{s'}$, which is then inserted into $Q$. If there are multiple shortest paths, then the state $s$ may produce $n \in \mathbb{N}^{\ge 1}$ successors generated by ideal preferred operators, which are all inserted into $\mathcal{Q}$. The successors not generated by preferred operators are ignored.

Now, consider the $(k+1)$-th expansion step. Since all the states in $\mathcal{Q}$ have equal and finite $h$-values, and GBFS tie-breaks by larger $g$-value, GBFS will expand one of the $n$ successors of $s$ generated by preferred operators over any other state in $\mathcal{Q}$, as they all have smaller or equal $g$-values than $g_{s}$. Of all the states in $\mathcal{Q}$, GBFS selects a state $t \in \mathcal{Q}$ to expand, one of the $n$ successors of $s$, with $g_{t} = g_{s} + 1$. Therefore, the perfect cost-to-goal estimate of state $s$ is $\hstarp{t} = \hstarp{s} - 1$, i.e., an expansion of a state generated by ideal preferred operators makes progress on an optimal path. %By the induction hypothesis, all states expanded in the first $k$ steps are part of an optimal path.

By expanding $t$ from $\mathcal{Q}$, GBFS continues the search along an optimal path until the goal condition is satisfied since the blind heuristic is goal-aware.
Given that the $(k+1)$-th expansion step satisfies the property, and we have shown the base case, we conclude that GBFS will expand at most the number of states part of an optimal path.
\end{proof}
