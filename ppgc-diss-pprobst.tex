...
\pp{Rewrote this according to Ritt's review.}

\section{Planning}
\label{sec:intro-planning}
Planning involves determining a series of operators (or actions) that transitions a given initial state to a desired goal state.\pp{Removed the citation here since we don't actually need it.}
...
Residual Neural Networks (ResNets)~\cite{He.etal/2016} can reduce the vanishing gradient problem\pp{Ritt put something here but I didn't understand what was written.}. ResNets use skip connections that bypass layers, allowing the information to flow directly from one layer to another. This bypassing mechanism mitigates the vanishing gradient problem and facilitates the training of deep networks.
