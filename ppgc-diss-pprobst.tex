...
\textit{Base case}: Initially, the only state in the preferred queue $\mathcal{Q}$ is the initial state $s_{0}$. GBFS expands states exclusively from the preferred queue. GBFS will start by expanding $s_{0}$. Since the task is solvable and GBFS does not ignore $s_{0}$ because the blind heuristic is safe, then $\hstarp{s_{0}} \neq \infty$. This expansion satisfies the property since $s_{0}$ is the only state expanded, and $s_{0}$ is part of any optimal path.
