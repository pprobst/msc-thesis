...
\begin{algorithm}
\caption{Sampling for preferred operators using \bfsrs}
\label{alg:sampling-po}
\begin{algorithmic}[1]
\Procedure{BRS}{$\Pi$, $N$, $k_{1}$, $k_{2}$}
  \State Initialize an empty queue $open$
  \State Initialize an empty set $S_{1}$
  \State \# First phase
  \State $S_{1} \gets$ BFS($s_{0}$, $open$, $k_1N$)
  \State Initialize an empty queue $closed$
  \State $closed \gets$ $S_{1}$
  \State Initialize an empty set $S_{2}$
  \State \# Second phase
...
\begin{algorithm}
\caption{Sampling with breadth-first search}
