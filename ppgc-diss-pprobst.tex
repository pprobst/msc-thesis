...
Assume that during the search, GBFS with the specified conditions expands a state $s$ that is not on an optimal path from $s_0$ to $s^*$. %Let us denote this non-optimal path as $\pi_s$ and its length as $|\pi_s|$.
