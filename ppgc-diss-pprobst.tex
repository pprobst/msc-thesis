...
\State Initialize empty array $S_{bfs}$
\State $S_{bfs}$, $Q \gets$ BFSLimited($s_{0}$, $p_{\bfsrw}N$)

\State Initialize empty array $S_{rw}$
\State \textbf{Phase 2}
\State Initialize Boolean array $V$ of size $|Q|$ with all indexes set to $False$
\While{$S_{bfs}\text{.size} + S_{rw}\text{.size} < N$}
    \State $rnd\_idx \gets$ random index of $Q$
    \While{$Q$[$rnd\_idx$] is $True$}
      \State $rnd\_idx \gets$ random index of $Q$
    \EndWhile
    ...
    \EndIf
    \State $S_{rw} \gets$ $S_{rw} + $ RW($s$, $L$, $S_{bfs}$) \emph{\# Random walk with regression limit $L$}
\EndWhile
\State $S \gets S_{bfs} + S_{rw}$
...
\State Initialize array $k$ with $s_{0}$ \emph{\# i.e., layer $k$}
\State Initialize empty array $k_{+1}$ \emph{\# i.e., layer $k+1$}
\State Initialize array $S_{bfs}$ with $s_{0}$
\While{$S_{bfs}\text{.size} < M$ and $k$ is not empty}
    \State Shuffle $k$
    \ForAll{partial states $s$ of $k$}
      \State Initialize empty array $succ_{s}$
      \ForAll{predecessor partial states $s'$ of $s$}
        \State Insert $s'$ into $succ_{s}$
      \EndFor
      \If{$S_{bfs}\text{.size} + succ_{s}\text{.size} \le  M$}
        \State Remove $s$ from $Q$
        \ForAll{partial states $s''$ of $succ_{s}$}
          \State Insert $s''$ into $S_{bfs}$, $k_{+1}$, $Q$
        \EndFor
      \EndIf
      \If{$S_{bfs}\text{.size} = M$}
        \State break
      \EndIf
    \EndFor
    \State $k \gets k_{+1}$
...
In addition to sampling the same states, it is common to sample neighboring states in the state space, especially for states close to the goal. We can leverage this to enhance the cost-to-goal estimates using the following approach.
...
\Procedure{BRS}{$\Pi$, $N$, $k_{1}$, $k_{2}$}
  ...
  \While{$S_{2}\text{.size} < k_{2}N$}
    \If{$open$ is empty}
      \State \textbf{return} $S_{1} + S_{2}$ (backward state space fully explored)
    \EndIf
    ...
    \State Insert $s$ into $closed$, $S_{2}$
    \ForAll{predecessor partial states $s'$ of $s$}
...
\Procedure{BFS}{$s_{0}$, $open$, $M$}
  ...
  \State Insert $s_{0}$ into $open$

  \While{$S_{1}\text{.size} < M$}
    \If{$open$ is empty}
