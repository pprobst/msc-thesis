...
\begin{itemize}
  \item Queremos aprender \alert{ações} que tornem a busca heurística mais \alert{eficiente} durante a resolução de uma tarefa de planejamento.
  \pause
...
\begin{itemize}
  \item Abordagem que depende \alert{muito} da lógica do domínio.
    \begin{itemize}
      ...
      \item e.g. (hyper)graph neural networks, action schema networks.
    \end{itemize}
  \pause
...
\begin{frame}{Gerando amostras para aprender funções heurísticas}
\begin{itemize}
...
&\ldots \\
\only<1>{&(\alert{14};0000000101011111\ldots10000010000100000) \\}
\only<2>{&(\textgreen{12};0000000101011111\ldots10000010000100000) \\}
&\ldots
\end{align*}
\end{frame}
