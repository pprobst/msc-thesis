...
i.e., \textblue{operadores aplicáveis} em $s$ que \textgreen{adicionam pelo menos um fato do gol}\footnote{Precondições de ações que adicionam um fato do gol \emph{também} são adicionadas em $S^{*}$ como ``sub-gols''.} no primeiro nível da solução relaxada.
\end{itemize}
\end{frame}
